...
import { Search, Trash2, RefreshCw, History, DollarSign, TrendingUp } from 'lucide-react';
...
const GAME_TYPES = [
  ...
  { value: 'whackamole', label: 'Whack-a-Mole' },
  { value: 'simon', label: 'Simon Says' },
];
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const enrichedHistory = (historyData || []).map(h => ({
        ...h,
        profiles: { username: profilesMap.get(h.user_id) || 'Unknown' }
      })) as HistoryEntry[];
      setHistory(historyData);

      // Calculate stats
      const totalBets = historyData.length;
      const totalWagered = historyData.reduce((sum, h) => sum + Number(h.bet_amount), 0);
      const totalPayout = historyData.reduce((sum, h) => sum + Number(h.payout), 0);
      const totalProfit = totalWagered - totalPayout; // House profit
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">View and manage all game bets</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-primary">${stats.totalWagered.toFixed(2)}</p>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
          <p className="text-sm text-muted-foreground">Total Payouts</p>
          <p className="text-2xl font-bold text-orange-500">${stats.totalPayout.toFixed(2)}</p>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <p className={`text-2xl font-bold ${stats.houseEdge >= 0 ? 'text-green-500' : 'text-red-500'}`}>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently delete {gameFilter === 'all' ? 'ALL' : gameFilter} game history.
            ...
            {loading ? (
              ...
              filteredHistory.map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell className="capitalize">{entry.game_type}</TableCell>
                  <TableCell className="text-right">${Number(entry.bet_amount).toFixed(2)}</TableCell>
                  <TableCell className="text-right">{Number(entry.multiplier).toFixed(2)}x</TableCell>
                  <TableCell className="text-right">${Number(entry.payout).toFixed(2)}</TableCell>
                  <TableCell className={`text-right font-semibold ${
                    Number(entry.profit) > 0 ? 'text-green-500' : Number(entry.profit) < 0 ? 'text-red-500' : ''
                  }`}>
